...
var example6 = require('./examples/person/6.json');
...
describe('person', function() {
  ...
  it('validates the fifth example', function() {
    var errors = dataModel.validate(example5);
    assert.equal(errors, null, JSON.stringify(errors));
  });

  it('validates the 6th example', function() {
    var errors = dataModel.validate(example6);
    assert.equal(errors, null, JSON.stringify(errors));
  });
